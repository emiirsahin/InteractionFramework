...
The included demo level is designed as a single-room apartment with everyday objects demonstrating the various capabilities of the framework, such as state transitions, requirement gating, focus-driven behavior, one-sided NPC interaction, and pickup-based progression.
...
The lamp switches between on and off states.
...
The vinyl player initially requires a vinyl record. Once the record is placed, it switches between on and off states.

3. **Focus-Driven Reveal**
An object starts with no prompt or interaction. When the player acquires a hidden key, focusing on the object later triggers a state change that enables interaction.
The water bottle can only be picked up after the plant requests it.
...
The ironing board requires both the shirt and the iron to be used, and it becomes non-interactable after use.
...
The plant guides the player on how to water it and has multiple states that each have a success and fail response depending on whether the player has fulfilled the requirement for that state.

7. **Pickup Interactions**
Pickupable objects grant keys and destroy themselves after use.
Examples of this interactable type are the vinyl record, the iron, and the shirt.
...
9. **Mixed Behavior**
Many other interaction-state-requirement configurations can be made with more complex behavior.
...
- **`InteractionComponent`:** Handles player-side interaction logic and focus detection.
- **`IInteractable`:** Interface implemented by all interactable actors.
- **`InteractableActorBase` and `InteractableNpcActorBase`:** Base classes that implement the `IInteractable` interface.
- **`InteractionDataAsset` and `NpcInteractionDataAsset`:** Define interaction states, requirements, and prompt data.
